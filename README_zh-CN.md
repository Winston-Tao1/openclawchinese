...
它在你已经使用的渠道（WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage, Microsoft Teams, WebChat）以及扩展渠道（BlueBubbles, Matrix, Zalo, 和 Zalo Personal）上回复你。它可以在 macOS/iOS/Android 上进行听说，并能渲染一个你控制的实时 Canvas。Gateway（网关）只是控制平面 —— 产品本身是这个助手。

如果你想要一个感觉本地化、快速且永远在线的单用户个人助手，这就是你的选择。
...
首选设置：运行入门向导 (`openclaw onboard`)。它会引导完成网关、工作区、渠道和技能的设置。CLI 向导是推荐路径，适用于 **macOS, Linux, 和 Windows (通过 WSL2; 强烈推荐)**。
支持 npm, pnpm, 或 bun。
...
模型说明：虽然支持任何模型，但我强烈推荐 **Anthropic Pro/Max (100/200) + Opus 4.5** 以获得长上下文优势和更好的提示注入防御。参见 [Onboarding](https://docs.openclaw.ai/start/onboarding)。
...
源码构建首选 `pnpm`。Bun 是可选的，用于直接运行 TypeScript。
...
- **[多 Agent 路由](https://docs.openclaw.ai/gateway/configuration)** — 将入站渠道/账户/对等端路由到隔离的 Agent（工作区 + 每个 Agent 的会话）。
- **[语音唤醒](https://docs.openclaw.ai/nodes/voicewake) + [对话模式](https://docs.openclaw.ai/nodes/talk)** — macOS/iOS/Android 上使用 ElevenLabs 的永远在线语音。
- **[实时 Canvas](https://docs.openclaw.ai/platforms/mac/canvas)** — 由 Agent 驱动的视觉工作区，使用 [A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)。
- **[一等公民工具](https://docs.openclaw.ai/tools)** — 浏览器, canvas, 节点, cron, 会话, 以及 Discord/Slack 操作。
- **[配套应用](https://docs.openclaw.ai/platforms/macos)** — macOS 菜单栏应用 + iOS/Android [节点](https://docs.openclaw.ai/nodes)。
- **[入门向导](https://docs.openclaw.ai/start/wizard) + [技能](https://docs.openclaw.ai/tools/skills)** — 带有捆绑/托管/工作区技能的向导式设置。
...
- [Gateway WS 控制平面](https://docs.openclaw.ai/gateway) 包含会话、在线状态、配置、cron、webhooks、[控制 UI](https://docs.openclaw.ai/web) 和 [Canvas 主机](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui)。
...
- [群组路由](https://docs.openclaw.ai/concepts/group-messages): 提及门控、回复标签、每渠道分块和路由。渠道规则: [Channels](https://docs.openclaw.ai/channels)。
...
- [Android 节点](https://docs.openclaw.ai/platforms/android): [Canvas](https://docs.openclaw.ai/platforms/mac/canvas), [对话模式](https://docs.openclaw.ai/nodes/talk), 相机, 屏幕录制, 可选 SMS。
...
- [技能平台](https://docs.openclaw.ai/tools/skills): 捆绑、托管和工作区技能，带有安装门控 + UI。
...
- **[Canvas + A2UI](https://docs.openclaw.ai/platforms/mac/canvas)** — Agent 驱动的视觉工作区 (A2UI 主机: [Canvas/A2UI](https://docs.openclaw.ai/platforms/mac/canvas#canvas-a2ui))。
- **[语音唤醒](https://docs.openclaw.ai/nodes/voicewake) + [对话模式](https://docs.openclaw.ai/nodes/talk)** — 永远在线的语音和连续对话。
- **[节点](https://docs.openclaw.ai/nodes)** — Canvas, 相机快照/剪辑, 屏幕录制, `location.get`, 通知, 加上 macOS 专用的 `system.run`/`system.notify`。
...
在小型 Linux 实例上运行 Gateway 是完全可以的。客户端（macOS 应用, CLI, WebChat）可以通过 **Tailscale Serve/Funnel** 或 **SSH 隧道** 连接，你仍然可以配对设备节点（macOS/iOS/Android）以便在需要时执行设备本地操作。
...
macOS 应用可以在 **node 模式** 下运行，并通过 Gateway WebSocket 广播其功能 + 权限映射 (`node.list` / `node.describe`)。客户端随后可以通过 `node.invoke` 执行本地操作：
...
- Gateway 通过 `sessions.patch` (WS 方法) 持久化每会话的切换状态，与 `thinkingLevel`, `verboseLevel`, `model`, `sendPolicy`, 和 `groupActivation` 一起。
...
## Agent 到 Agent (sessions\_\* 工具)

- 使用这些工具在会话之间协调工作，而无需在聊天界面之间跳转。
- `sessions_list` — 发现活动会话 (agents) 及其元数据。
- `sessions_history` — 获取会话的记录日志。
- `sessions_send` — 给另一个会话发送消息; 可选的回复乒乓 + 公告步骤 (`REPLY_SKIP`, `ANNOUNCE_SKIP`)。
...
仅 Gateway 就能提供出色的体验。所有应用都是可选的，并增加额外的功能。
...
- **默认:** 工具在 **main** 会话的主机上运行，所以当只有你使用时，Agent 拥有完全访问权限。
- **群组/渠道安全:** 设置 `agents.defaults.sandbox.mode: "non-main"` 以在每会话 Docker 沙箱内运行 **非 main 会话** (群组/渠道); bash 随后在这些会话的 Docker 中运行。
...
- 可选: 设置 `channels.telegram.groups` (带有 `channels.telegram.groups."*".requireMention`); 设置后，它是群组允许列表 (包含 `"*"` 以允许所有)。根据需要还可以设置 `channels.telegram.allowFrom` 或 `channels.telegram.webhookUrl` + `channels.telegram.webhookSecret`。
...
- 仅限 macOS 的旧版集成，通过 `imsg` (Messages 必须已登录)。
...
<p align="left">
  ...
  <a href="https://github.com/sergical"><img src="https://avatars.githubusercontent.com/u/3760543?v=4&s=48" width="48" height="48" alt="sergical" title="sergical"/></a> <a href="https://github.com/shiv19"><img src="https://avatars.githubusercontent.com/u/9407019?v=4&s=48" width="48" height="48" alt="shiv19" title="shiv19"/></a> <a href="https://github.com/shiyuanhai"><img src="https://avatars.githubusercontent.com/u/1187370?v=4&s=48" width="48" height="48" alt="shiyuanhai" title="shiyuanhai"/></a> <a href="https://github.com/siraht"><img src="https://avatars.githubusercontent.com/u/73152895?v=4&s=48" width="48" height="48" alt="siraht" title="siraht"/></a> <a href="https://github.com/snopoke"><img src="https://avatars.githubusercontent.com/u/249606?v=4&s=48" width="48" height="48" alt="snopoke" title="snopoke"/></a> <a href="https://github.com/techboss"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="techboss" title="techboss"/></a> <a href="https://github.com/testingabc321"><img src="https://avatars.githubusercontent.com/u/8577388?v=4&s=48" width="48" height="48" alt="testingabc321" title="testingabc321"/></a> <a href="https://github.com/search?q=The%20Admiral"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="The Admiral" title="The Admiral"/></a> <a href="https://github.com/thesash"><img src="https://avatars.githubusercontent.com/u/1166151?v=4&s=48" width="48" height="48" alt="thesash" title="thesash"/></a> <a href="https://github.com/search?q=Vibe%20Kanban"><img src="assets/avatar-placeholder.svg" width="48" height="48" alt="Vibe Kanban" title="Vibe Kanban"/></a>
